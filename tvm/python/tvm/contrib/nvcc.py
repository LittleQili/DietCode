...
def compile_cuda(code, target="ptx", arch=None, options=None, path_target=None):
    ...
    # <bojian/DietCode>
    if int(os.getenv("DIETCODE_CHECK_REGISTER_SPILL", "0")):
        print("Verifying dispatch. Forcing target to be cubin")
        target = "cubin"
    ...
    # <bojian/DietCode>
    import re
    reg_info_pattern = re.compile("    ([0-9]+) bytes stack frame, ([0-9]+) bytes spill stores, ([0-9]+) bytes spill loads")
    reg_info = reg_info_pattern.findall(py_str(out))
    if len(reg_info) != 0:
        if int(os.getenv("DIETCODE_CHECK_REGISTER_SPILL", "0")) and \
           reg_info[0] != ('0', '0', '0'):
            raise RuntimeError("Register spilling of {} bytes is deemed as " \
                               "failed compilation".format(reg_info))
